...
class RsyncDir(object):
    ...
    def backup(self):
        ...
        try:
            if self.deleteflag:
                subprocess.call("rsync --stats --progress --partial -h -avrlLH --delete-after -e ssh {localdir} {remoteuser}@{remotehost}:{remotedir}".format(**self.__dict__), shell=True)
            else:
                subprocess.call("rsync --stats --progress --partial -h -avrlLH -e ssh {localdir} {remoteuser}@{remotehost}:{remotedir}".format(**self.__dict__), shell=True)
            t2 = time.time()
